<html>
  ...
  <body style="margin: 0px; padding:0px; font-family: arial;">
    ...
    <div id="test" style="height:100%; width:100%; position:relative; ">
      ...
      <div style="position:absolute; top:0px; left:65px; font-size:18px;">
        <b><span id="out2"></span> </b><span style="font-size:12px;"><?php echo _("(Hover for info, press to zoom in. Histogram: "); ?>
      ...
      <b>
      <p style="position:absolute; top: 200px; left:0px;">
        <span id="axislabely"></span>
      </p>
      <p style="position:absolute; bottom: 40px; left:450px;">
        <span id="axislabelx">Date / Time</span>
      </p></b>
    ...
    <script id="source" language="javascript" type="text/javascript">var kwhd =<?php echo $kwhd; ?>
  ;
  var whw = 
<?php echo $whw; ?>
  ;
...
<?php echo $apikey; ?>
  ...
  bot_kwhd_text =
<?php echo _("Total: "); ?>+(tkwh).toFixed(0)+<?php echo _(" kWh : pounds"); ?>+(tkwh*price).toFixed(0) + <?php echo _(" | Average: "); ?>+(tkwh/ndays).toFixed(1)+ <?php echo _(" kWh : pounds"); ?>+((tkwh/ndays)*price).toFixed(2)+" | £"+((tkwh/ndays)*price*7).toFixed(0)+<?php echo _(" a week, pounds"); ?>+((tkwh/ndays)*price*365).toFixed(0)+<?php echo _(" a year | Unit price: pounds"); ?>+price;
...
alert(<?php echo _("The whw parameter must be set in the dashboard to a histogram feed id."); ?>
  ...
  function on_inst_graph_load()
  {
    ...
    var datetext = "";
    if ((end - start) < 3600000 * 25)
    {
      var mdate = new Date(start);
      datetext = " | " + mdate.format("dS mmm yyyy")
    }
