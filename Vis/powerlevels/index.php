...
-->
  <?php  
    $path = dirname("http://".$_SERVER['HTTP_HOST'].str_replace('Vis/powerlevels', '', $_SERVER['SCRIPT_NAME']))."/";
  ...
  <body style="margin: 0px; padding:10px; font-family: arial; background-color:rgb(245,245,235);">
    ...
    <script id="source" language="javascript" type="text/javascript">
      ...
      $.ajax({                                      
        ...
        data: "&apikey="+apikey+"&id="+feedid+"&min="+0+"&max="+threshold,
        dataType: 'json',
        async: false,                      
      ...
      $.ajax({                                      
        url: path+'feed/kwhatpower.json',                         
        data: "&apikey="+apikey+"&id="+feedid+"&min="+(threshold+1)+"&max="+maxrange,
        dataType: 'json',
        async: false,                      
