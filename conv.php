<?php
  ...
  */
  ...
  foreach ($feeds as $feed ){
    ...
    $time = 0; $data = 0; $data2 = 0;
    $result = db_query("SHOW COLUMNS FROM $feedname");
    if ($result==false) {echo ", but there is an entry in the feeds table?"; die;}
    $row = db_fetch_array($result);
    if ($row[0]=="time") {$time = 1; $timeformat = $row['Type'];}
    $row = db_fetch_array($result);
    if ($row[0]=="data") $data = 1;
    $row = db_fetch_array($result);
    if ($row[0]=="data2") $data2 = 1;

    if (!$time) {echo "table $feedname does not have a time field?"; die;}
    if (!$data) {echo "table $feedname does not have a data field?"; die;}

    if ($timeformat=="datetime")
    {
      $result = db_query("SHOW TABLE STATUS LIKE '$feedname'");
      $row = db_fetch_array($result);
      $rows = $row['Rows']; echo $rows;
      ...
      $result = db_query("SHOW TABLE STATUS LIKE '$feedname'");
      $row = db_fetch_array($result);
      if ($rows!=$row['Rows']) { echo "Number of rows in converted table is different from original, original data can be found in the feed_tmp table"; die; }
