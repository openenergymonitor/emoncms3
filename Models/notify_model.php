<?php
  ...
  */
  ...
  function run_notify($userid)
  {
    ...
    $notification_message = "";									// Variable holds body of email

    $result = db_query("SELECT * FROM notify WHERE `userid` = '$userid'");			// Get user notification list
    while($row = db_fetch_array($result))							// For each feed in the notification list
    {
      $feedid = $row['feedid'];
      $feed = get_feed($feedid);								// Get all feed details from feed model function
	
      if ((time() - strtotime($feed[3]))<(60*1)) $active = 1; else $active = 0;			// Check if feed is active
	

      // if feed becomes active again then the notification status needs to be reset to unsent
      if ($active && $row['oninactive_sent']) db_query("UPDATE notify SET oninactive_sent = 0 WHERE feedid='$feedid'"); 

      // if feed is not the onvalue value then notification status needs to be reset to unsent
      if ($feed[4] != $row['onvalue'] && $row['onvalue_sent']) db_query("UPDATE notify SET onvalue_sent = 0 WHERE feedid='$feedid'"); 		
    ...
    if ($notification_number>0) send_mail($to,$subject,$body);
  ...
  function set_notify($userid,$feedid,$onvalue,$oninactive,$periodic)
  {
    ...
    if ($row = db_fetch_array($result)) {
      ...
      db_query("INSERT INTO notify (`userid`,`feedid`,`onvalue`,`onvalue_sent`,`oninactive`,`oninactive_sent`,`periodic`) VALUES ('$userid','$feedid','$onvalue','$onvalue_sent','$oninactive','$oninactive_sent','$periodic')");
    }
  }
