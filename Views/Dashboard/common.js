...
function draw_dials()
{
  $('.dial').each(function(index)
  {
    ...
    if (!$(this).html())
    {
      // Only calling this when its empty saved a lot of memory! over 100Mb
      $(this).html('<canvas id="' + id + '" width="200px" height="160px"></canvas>');
      firstdraw = 1;
    }

    if ((val * 1).toFixed(1) != (assoc[feed] * 1).toFixed(1) || firstdraw == 1)
    {
      //Only update graphs when there is a change to update
  ...
  $('.centredial').each(function(index)
  {
    ...
    if ((val * 1).toFixed(1) != (assoc[feed] * 1).toFixed(1) || firstdraw == 1)
    {
      ...
      var ctx = canvas.getContext("2d");
      draw_centregauge(ctx, 200 / 2, 100, 80, val * scale, maxval, units);
      firstdraw = 0;
    }
  });
}
...
function draw_leds()
{
  ...
  $('.binaryled').each(function(index)
  {
    ...
    var circle = canvas.getContext("2d");
    if (browserVersion < 9)
      draw_binary_led_ie8(circle, val);
    else
      draw_binary_led(circle, val);
    firstdraw = 0;
    //  }
  });
}
...
function draw_graphs()
{
  $('.graph').each(function(index)
  {
    ...
    var start = ((new Date()).getTime()) - timeWindow;
    //Get start time
